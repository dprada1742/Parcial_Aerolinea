...
describe('AerolineaService', () => {
  ...
  it('findOne should throw an exception for an invalid aerolinea', async () => {
    await expect(() => service.findOne("0")).rejects.toHaveProperty("message", "La aerolínea con el id dado no fue encontrada")
  });
  ...
  it('create should throw an exception for a fecha fundacion in the future', async () => {
    const aerolinea: AerolineaEntity = {
      id: "",
      nombre: faker.company.name(), 
      descripcion: faker.lorem.sentence(), 
    ...
    await expect(() => service.create(aerolinea)).rejects.toHaveProperty("message", "La fecha de fundación debe ser en el pasado")
  });

  it('update should modify a aerolinea', async () => {
    const aerolinea: AerolineaEntity = aerolineasList[0];
    aerolinea.nombre = "New name";
    aerolinea.paginaWeb = "http://new-website.com";
  ...
  it('update should throw an exception for an invalid aerolinea', async () => {
    ...
    aerolinea = {
      ...aerolinea, nombre: "New name", paginaWeb: "http://new-website.com"
    }
    await expect(() => service.update("0", aerolinea)).rejects.toHaveProperty("message", "La aerolínea con el id dado no fue encontrada")
  });
  ...
  it('delete should throw an exception for an invalid aerolinea', async () => {
    const aerolinea: AerolineaEntity = aerolineasList[0];
    await service.delete(aerolinea.id);
    await expect(() => service.delete("0")).rejects.toHaveProperty("message", "La aerolínea con el id dado no fue encontrada")
  });
});